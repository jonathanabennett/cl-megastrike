...
(defun format-move (element)
  (let ((mv-string ""))
    (if (eql (prime-move element) :walk)
        (concatenate 'string mv-string (format nil "~a" (prime-distance element))))
    (if (eql (secondary-move element) :jump)
        (concatenate 'string mv-string (format nil "/~aJ" (secondary-distance element))))
    mv-string))
