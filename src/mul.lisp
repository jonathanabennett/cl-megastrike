...
(defun parse-csv (header-row row)
  (let ((data (loop for key in header-row
    ...
    (setf (cdr (assoc 'movement data)) (move-from-string (cdr (assoc 'movement data))))
  ...
  (let ((mv-alist '())
    ...
    (dolist (str mv-strings)
      (multiple-value-bind (dist type) (extract-numbers-and-letters str)
        (cond
          ((string= "j" type)
           (setf mv-alist (acons :jump (/ (parse-integer dist) 2) mv-alist)))
          ((string= "qt" type)
           (setf mv-alist (acons :tracked-quadvee (/ (parse-integer dist) 2) mv-alist)))
          ((string= "qw" type)
           (setf mv-alist (acons :wheeled-quadvee (/ (parse-integer dist) 2) mv-alist)))
          ((string= "t" type)
           (setf mv-alist (acons :tracked (/ (parse-integer dist) 2) mv-alist)))
          ((string= "w" type)
           (setf mv-alist (acons :wheeled (/ (parse-integer dist) 2) mv-alist)))
          ((string= "h" type)
           (setf mv-alist (acons :hover (/ (parse-integer dist) 2) mv-alist)))
          ((string= "v" type)
           (setf mv-alist (acons :vtol (/ (parse-integer dist) 2) mv-alist)))
          ((string= "n" type)
           (setf mv-alist (acons :naval (/ (parse-integer dist) 2) mv-alist)))
          ((string= "s" type)
           (setf mv-alist (acons :submarine (/ (parse-integer dist) 2) mv-alist)))
          ((string= "f" type)
           (setf mv-alist (acons :foot-inf (/ (parse-integer dist) 2) mv-alist)))
          ((string= "m" type)
           (setf mv-alist (acons :mot-inf (/ (parse-integer dist) 2) mv-alist)))
          ((string= "g" type)
           (setf mv-alist (acons :wige (/ (parse-integer dist) 2) mv-alist)))
          ((string= "a" type)
           (setf mv-alist (acons :aerodyne (/ (parse-integer dist) 2) mv-alist)))
          ((string= "p" type)
           (setf mv-alist (acons :spheroid (/ (parse-integer dist) 2) mv-alist)))
          (t
           (setf mv-alist (acons :walk (/ (parse-integer dist) 2) mv-alist))))))
    mv-alist))
