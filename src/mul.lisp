...
(defun mul-parser (header-row row)
  (let ((uuid (uuid:make-v1-uuid))
    ...
    (setf (gethash uuid *mul*) (new-mek (assoc 'chassis data)
                                        (assoc 'model data)
                                        (assoc 'role data)
                                        (assoc 'unit-type data)
                                        (assoc 'size data)
                                        (assoc 'movement data)
                                        (assoc 'tmm data)
                                        (assoc 'armor data)
                                        (assoc 'structure data)
                                        (assoc 'threshold data)
                                        (assoc 'short data)
                                        (assoc 'short* data)
                                        (assoc 'medium data)
                                        (assoc 'medium* data)
                                        ...
                                        (assoc 'ov data)
                                        (assoc 'pv data)
                                        (assoc 'abilities data)
                                        (assoc 'front-arc data)
                                        (assoc 'left-arc data)
                                        (assoc 'right-arc data)
                                        (assoc 'rear-arc data)))))
